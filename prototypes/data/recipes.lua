...
for h = -20, 340, 20 do
    ...
    if data.raw.tile["concrete"] then
        local color_concrete_recipe =
            table.deepcopy(data.raw.recipe["concrete"])

        color_concrete_recipe.name = color_name .. "-concrete"
        color_concrete_recipe.category = "crafting"
        color_concrete_recipe.energy_required = 0.1
        color_concrete_recipe.enabled = false
        color_concrete_recipe.ingredients = {
            {type = "item", name = "concrete", amount = 1}
        }

        color_concrete_recipe.results = {
            {type = "item", name = color_name .. "-concrete", amount = 1}
        }
        color_concrete_recipe.main_product = color_name .. "-concrete"

        data:extend({color_concrete_recipe})
    end
end
